...
if (!isset($_SESSION['user_id'])) {
    echo "<p style='padding:20px;'>Please <a href='login.php'>log in</a> to view your cart.</p>";
    exit();
}

$user_id = intval($_SESSION['user_id']);
$query = "SELECT * FROM food_items WHERE cart = 1 AND cart_user_id = $user_id";
...
<head>
    ...
    </style> <!-- If your main styles are in a CSS file -->
</head>
<body>
    <section class="foods-section">
        <h2 class="foods-title">Your Cart</h2>
        ...
        button.addEventListener('click', function() {
            ...
            .then(data => {
                if (data.trim() === 'success') {
                    card.remove(); // Remove the item from the page
                    if (!document.querySelector('.food-card')) {
                        document.getElementById('foodsGrid').innerHTML = "<p style='padding:20px;'>Your cart is empty.</p>";
                    }
                } else {
